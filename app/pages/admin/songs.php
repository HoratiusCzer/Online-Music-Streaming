    ...
    if($action == 'add')
    {
        ...
        if($_SERVER['REQUEST_METHOD'] == "POST")
        {
            ...
            if(empty($errors))
            {
                ...
                $query = "insert into songs (title,image,file,user_id,category_id,artist_id,date,views,slug,lyrics) values (:title,:image,:file,:user_id,:category_id,:artist_id,:date,:views,:slug,:lyrics)";
    ...
    if($action == 'edit')
    {
        ...
        if($_SERVER['REQUEST_METHOD'] == "POST" && $row)
        {
            ...
            if(empty($errors))
            {
                ...
                $query = "update songs set title = :title,user_id =:user_id,category_id =:category_id,artist_id =:artist_id,lyrics = :lyrics";
    ...
    <?php if($action == 'add'):?>
        ...
        <div style="max-width: 500px;margin: auto;">
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="form-control my-1">
                    <div>Lyrics:</div>
                    <textarea name="lyrics" class="form-control my-1" placeholder="Enter the lyrics of the song"><?=set_value('lyrics')?></textarea>
                    <?php if(!empty($errors['lyrics'])):?>
                        <small class="error"><?=$errors['lyrics']?></small>
                    <?php endif;?>
                </div>
        ...
        <div style="max-width: 500px;margin: auto;">
            <form method="post" enctype="multipart/form-data">
                ...
                <?php if(!empty($row)):?>
                    ...
                    <!-- Lyrics Section for Edit -->
                    <div class="form-control my-1">
                        <div>Lyrics:</div>
                        <textarea name="lyrics" class="form-control my-1" placeholder="Enter the lyrics of the song"><?=set_value('lyrics', $row['lyrics'])?></textarea>
                        <?php if(!empty($errors['lyrics'])):?>
                            <small class="error"><?=$errors['lyrics']?></small>
                        <?php endif;?>
                    </div>
